...
def run_animation(sender, app_data):
    subprocess.run(["python", "animationNe.py"])
...
def search(sender, app_data, user_data):
	...
	for index, song in enumerate(songs):
		if app_data in song.lower():
			dpg.add_button(label=f"{ntpath.basename(song)}", callback=play,width=-1, height=35, user_data=song, parent="list")
			dpg.add_spacer(height=2,parent="list")
def removeall():
...
with dpg.theme(tag="base"):
	with dpg.theme_component():
		dpg.add_theme_color(dpg.mvThemeCol_Button, (0, 0, 128))
		dpg.add_theme_color(dpg.mvThemeCol_ButtonActive, (137, 142, 255, 95))
		dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered, (137, 142, 0))
		...
		dpg.add_theme_color(dpg.mvThemeCol_ChildBg, (100,0,0))
...
with dpg.theme(tag="songs"):
	with dpg.theme_component():
		...
		dpg.add_theme_color(dpg.mvThemeCol_Button, (0, 0, 128, 150))
		dpg.add_theme_color(dpg.mvThemeCol_ButtonHovered, (255,255,0,90))
...
with dpg.window(tag="main",label="window title"):
	...
	with dpg.group(horizontal=True):
		with dpg.child_window(width=250,tag="sidebar"):
			dpg.add_text("RAINY MUSICART________",color=(255,255,0))
			dpg.add_text("Build by Tran Huu Thong")
			dpg.add_text("and Dang Thi Kim Loan.")
			dpg.add_separator()
			dpg.add_spacer(height=5)
			dpg.add_button(label="Support (Facebook)",width=-1,height=33,callback=lambda:webbrowser.open(url="https://www.facebook.com/huuthong293dng"))
			dpg.add_spacer(height=5)
			dpg.add_separator()
			dpg.add_spacer(height=5)
			dpg.add_button(label="Add File :>>",width=-1,height=33,callback=add_files)
			dpg.add_spacer(height=6)
			dpg.add_button(label="Add Folder :>>",width=-1,height=33,callback=add_folder)
			dpg.add_spacer(height=6)
			dpg.add_button(label="Remove All Songs :<",width=-1,height=33,callback=removeall)
			dpg.add_spacer(height=6)
			dpg.add_separator()
			dpg.add_spacer(height=5)
			...
			dpg.add_text("     (>.<@)",color=(255,255,0))
			dpg.add_text("      Thanks ",color=(255,255,255))
			dpg.add_text("       for ",color=(0,0,255))
			dpg.add_text("        listening ",color=(128,128,0))
			dpg.add_text("         to ",color=(255,255,0))
			dpg.add_text("          my ",color=(0,0,0))
			dpg.add_text("           music ",color=(0,0,255))
			dpg.add_text("            <3 ",color=(128,128,0))
			dpg.add_text("             . ",color=(255,255,0))
		...
		with dpg.child_window(autosize_x=True,border=False):
			with dpg.child_window(autosize_x=True,height=80,no_scrollbar=True):
				with dpg.group(horizontal=True):
					with dpg.group(horizontal=True):
						dpg.add_button(label=" ~~  Play",width=150,height=30,tag="play",callback=play_pause)
						dpg.add_button(label=" <<  (Pre)",width=150,height=30,show=True,tag="pre",callback=pre)
						dpg.add_button(label=" >>  (Next)",tag="next",show=True,callback=next,width=150,height=30)
						dpg.add_button(label=" ||  (Stop)",callback=stop,width=150,height=30)
						dpg.add_button(label="Run Animation", width=300, height=30, callback=run_animation)
